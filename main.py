...
def _build_oauth_header(method: str, url: str, body_params: dict | None = None) -> str:
    ...
    header_parts = ", ".join(f'{_percent_encode(k)}="{_percent_encode(v)}"' for k, v in sorted(oauth_params.items()))
...
def health():
    ...
    return {"status": "ok" if configured else "unconfigured", "x_api_configured": configured}
...
async def create_post(payload: PostRequest, request: Request):
    ...
    if len(tweet_text) > 280:
        ...
        if payload.hashtags:
            suffix += f"\n\n{payload.hashtags}"
        max_body = 280 - len(reserved) - len(suffix) - 3
        if max_body > 20:
            tweet_text = f"{reserved}{payload.body[:max_body].rstrip()}...{suffix}"
        else:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(url, json={"text": tweet_text}, headers={"Authorization": auth_header, "Content-Type": "application/json"})
        if response.status_code == 201:
            data = response.json()
            tweet_id = data.get("data", {}).get("id", "unknown")
            logger.info("Tweet posted successfully: %s", tweet_id)
            return {"external_post_id": tweet_id, "status": "published", "message": f"Posted to X: https://x.com/politmatrader/status/{tweet_id}"}
        else:
            error_detail = response.text[:500]
            logger.error("X API error %s: %s", response.status_code, error_detail)
            raise HTTPException(status_code=502, detail=f"X API returned {response.status_code}: {error_detail}")
